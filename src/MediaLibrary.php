...
class MediaLibrary
{
        ...
        switch ($status) {
            case 'completed':
                ...
                echo '<span class="tinify-status tinify-done">'
                   . esc_html(sprintf(__('✓ %s%% smaller', 'tinify-ai'), round((float) $pct, 1)))
                   . '</span>';
                break;
            case 'processing':
            case 'pending':
        ...
        <div class="misc-pub-section tinify-panel">
            ...
            <?php if ($status === 'completed'): ?>
                ...
                <?php if ($origSize && $procSize): ?>
                    <?php printf(esc_html__('%1$s → %2$s', 'tinify-ai'),
                        esc_html(size_format((int) $origSize)),
                        esc_html(size_format((int) $procSize))); ?><br>
                <?php endif; ?>
    ...
    public function handleAjaxOptimizeSingle(): void
    {
        ...
        if (!$attachmentId) wp_send_json_error('Invalid attachment ID');
