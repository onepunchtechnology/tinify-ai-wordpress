		...
		<div class="wrap">
			...
			<p>
				...
				printf(
					...
					esc_html__('%1$s plan · %2$s credits remaining', 'tinify-ai'),
			...
			<p>
				...
				printf(esc_html__('✓ Optimized: %d', 'tinify-ai'), absint($counts['completed']));
				?>
				</strong>
				&nbsp;|&nbsp;
				<?php
				/* translators: %d: count of pending images */
				printf(esc_html__('Pending: %d', 'tinify-ai'), absint($counts['pending'] + $counts['processing']));
				?>
				&nbsp;|&nbsp;
				<?php
				/* translators: %d: count of failed images */
				printf(esc_html__('Failed: %d', 'tinify-ai'), absint($counts['failed']));
				?>
			</p>
			...
			<div class="notice notice-warning inline">
				...
				printf(
					...
					'<a href="https://tinify.ai/pricing" target="_blank">' . esc_html__('Upgrade', 'tinify-ai') . '</a>' // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				);
				?>
				</p>
			</div>
			...
			<button id="tinify-bulk-start" class="button button-primary">
				...
				printf(esc_html__('Optimize %d images', 'tinify-ai'), absint(max(0, $counts['total'] - $counts['completed'])));
				?>
			</button>
			...
			<button id="tinify-bulk-retry" class="button" style="margin-left:8px;">
				<?php
				/* translators: %d: count of failed images */
				printf(esc_html__('Retry %d failed', 'tinify-ai'), absint($counts['failed']));
				?>
			</button>
			<?php endif; ?>
		</div>
